...
function Item({ to, children }){
  return (
    <NavLink
      to={to}
      className={({ isActive }) => `block rounded px-3 py-2 text-sm ${isActive ? 'bg-white/10 text-text-primary' : 'text-text-secondary hover:text-text-primary hover:bg-white/5'}`}
    >
      {children}
    </NavLink>
  )
...
export default function Sidebar({ bare = false }){
  ...
  useEffect(()=>{
    try{ const v = localStorage.getItem('arc_sidebar'); if(v!==null) setCollapsed(v==='1') }catch{}
  },[])
  function toggle(){
    setCollapsed(c=>{ const n=!c; try{ localStorage.setItem('arc_sidebar', n?'1':'0') }catch{}; return n })
  }
  const content = (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {!bare && (
            <button onClick={toggle} className="text-xs text-text-secondary hover:text-text-primary border border-white/10 rounded px-2 py-0.5">
        ...
        <div>
          <button onClick={logout} className="text-sm text-text-secondary hover:text-text-primary">Logout</button>
        </div>
      </div>
  )
  if(bare) return content
  return (
    <aside className={`shrink-0 border-r border-white/10 hidden md:block p-3 transition-all ${collapsed? 'w-40' : 'w-64'}`}>
      <div className={collapsed? 'text-sm' : ''}>
        {content}
      </div>
    </aside>
  )
