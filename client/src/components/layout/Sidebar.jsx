import { ChevronsLeft, LogOut, Home, Clock, Archive, User, Tag, BarChart2, Trophy, Grid, PlusCircle } from 'lucide-react'
import React, { useState, useEffect, useRef, useContext } from 'react';

// Cache of avatar URLs that have already finished loading during this session.
...
function Item({ to, icon: Icon, children, expanded, end = false, compact = false, mobile = false }) {
  ...
  return (
    <NavLink
      ...
      onClick={() => { if (mobile) mobileClose?.() }}
      className={({ isActive }) => {
        const base = isActive
          ? 'text-primary bg-primary/15'
          : 'text-text-secondary hover:text-text-primary hover:bg-surface-muted'
        return `group relative flex items-center gap-3 rounded-lg ${padding} ${textClass} transition-colors ${!expanded ? 'justify-center' : ''} ${base}`
      }}
...
function Heading({ label, top = false, expanded, compact = false }) {
  ...
  if (!expanded) {
    return <div className={`${topMargin} mb-4`}><hr className="mx-auto w-1/2 border-soft" /></div>;
  }

  // When expanded, render the text title
  return (
    <div className={`${topMargin} mb-2 px-3 text-xs font-semibold tracking-wider text-text-tertiary uppercase text-left`}>{label}</div>
  )
}

function SidebarContent({ expanded, onLogout, onLock, onUnlock, user, isAdmin, isLocked, compact = false, mobile = false }) {
  ...
  const avatarUrl = user?.avatar || `https://i.pravatar.cc/150?u=${user?.email}`
  const [avatarLoaded, setAvatarLoaded] = useState(() => avatarLoadedCache.has(avatarUrl))
  return (
    <div className={`flex flex-col h-full ${padding}`}>
      {/* Header */}
      <div className={`flex items-center ${mobile ? 'mb-4' : 'mb-6'} ${expanded ? 'justify-between' : 'justify-center'}`}>
      ...
      {/* Nav Items */}
      <div className={`flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar ${compact ? '-mr-1 pr-1' : '-mr-2 pr-2'}`}>
        {user && !isAdmin && (
          <>
            <Item to="/dashboard" icon={Home} expanded={expanded} end mobile={mobile}>{'Dashboard'}</Item>
            <Item to="/test/today" icon={Clock} expanded={expanded} mobile={mobile}>{"Today's RC"}</Item>
            <Item to="/archive" icon={Archive} expanded={expanded} mobile={mobile}>{'Archive'}</Item>

            <Heading label="Insights" expanded={expanded} />
            <Item to="/results" icon={BarChart2} expanded={expanded} end compact={compact} mobile={mobile}>{'Results'}</Item>
            <Item to="/performance" icon={Grid} expanded={expanded} compact={compact} mobile={mobile}>{'Performance Studio'}</Item>

            <Heading label="Leaderboard" expanded={expanded} />
            <Item to="/leaderboard/global" icon={Trophy} expanded={expanded} compact={compact} mobile={mobile}>{'Global'}</Item>
            <Item to="/leaderboard/local" icon={Trophy} expanded={expanded} compact={compact} mobile={mobile}>{'Cohort'}</Item>
          </>
        )}
        {user && isAdmin && (
          <>
      ...
      {/* Footer */}
      <div className={`mt-auto border-t border-soft flex flex-col items-stretch ${compact ? 'py-6 gap-4' : 'py-6 gap-2'}`}>
        {/* Logout button first */}
        {/* <div className="px-3"> */}
        <button
          onClick={onLogout}
          className={`group relative flex items-center gap-4 rounded-lg w-full px-3 py-2.5 text-sm transition-colors text-text-secondary hover:text-error hover:bg-error/10 ${!expanded ? 'justify-center' : ''}`}
          title="Logout"
          aria-label="Logout"
        >
        ...
        <div className={`px-3 flex items-center gap-3 ${expanded ? '' : 'justify-center'}`}>
          <div className="relative w-10 h-10 flex-none shrink-0">
            {/* Placeholder skeleton while avatar loads */}
            {!avatarLoaded && (
            ...
            <img
              ...
              fetchPriority="high"
              onLoad={() => {
                avatarLoadedCache.add(avatarUrl)
                setAvatarLoaded(true)
              }}
              onError={() => {
                // Prevent endless retry/skeleton; mark as loaded so placeholder hides
                avatarLoadedCache.add(avatarUrl)
                setAvatarLoaded(true)
              }}
              className={`w-10 h-10 rounded-full object-cover transition-opacity duration-150 ${avatarLoaded ? 'opacity-100' : 'opacity-0'}`}
            />
          </div>
          {expanded && (
...
export default function Sidebar({ bare = false, mobile = false, compact = false }) {
  ...
  const [mode, setMode] = useState(() => {
    try { return localStorage.getItem('arc_sidebar_mode') === 'locked' ? 'locked' : 'collapsed' } catch { return 'locked' }
  })
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--arc-sidebar-width', locked ? expandedWidth : railWidth)
  }, [locked])
  ...
  if (bare) {
    return (
      <div className="bg-card-surface" style={{ width: expandedWidth }}>
        <SidebarContent expanded user={user} isAdmin={isAdmin} onLogout={logout} isLocked={true} onUnlock={closeSidebar} mobile={mobile} compact={compact && !mobile} />
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <aside
        onMouseEnter={() => { if (!locked && !suppressHoverRef.current) setHovering(true) }}
        onMouseLeave={() => { if (!locked) { setHovering(false); suppressHoverRef.current = false } }}
        className="hidden md:block bg-card-surface"
        style={{ position: 'fixed', left: 0, top: 0, bottom: 0, width: railActualWidth, zIndex: 40, transition: 'width .2s ease-in-out' }}
      >
        <SidebarContent {...commonProps} expanded={locked} isLocked={locked} mobile={mobile} compact={compact && !mobile} />
      </aside>
      ...
      <div
        className={`hidden md:block bg-card-surface shadow-lg fixed top-0 bottom-0 z-50 transition-transform duration-200 ease-in-out ${
          ...
          slidingBackVisible ? (slidingBackOut ? '-translate-x-full' : 'translate-x-0') : (showOverlay ? 'translate-x-0' : '-translate-x-full')
          }`}
        style={{ width: expandedWidth }}
        onMouseLeave={() => { setHovering(false); suppressHoverRef.current = false }}
        onMouseEnter={() => setHovering(true)}
      >
        <SidebarContent {...commonProps} expanded isLocked={false} mobile={mobile} compact={compact && !mobile} />
      </div>
    </>
  )
}
