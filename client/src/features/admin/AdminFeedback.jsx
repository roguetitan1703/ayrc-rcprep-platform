        ...
        {type === 'multi' && (
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => handleChange('options', e.target.value.split(','), isNew)}
      ...
      <div key={q.id || q._id} className="bg-card-surface p-4 rounded shadow space-y-2">
        {isEditing ? (
          <>
            ...
            <Button onClick={() => handleUpdate(q.id)}>Save</Button>
            <Button onClick={() => setEditingQuestion(null)}>Cancel</Button>
          </>
        ) : (
          <>
            <div>
              <strong>{q.label || 'No label'}</strong> ({q.type || 'rating'}) — Time: {q.time || 0}s
              — Date:{' '}
              {q.date
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="space-y-4">
        {todaysQuestions.length > 0 ? (
          todaysQuestions.map(renderQuestionCard)
        ) : (
          <div>No questions for today.</div>
        )}
      </div>
      ...
      <div className="space-y-4">
        {futureQuestions.length > 0 ? (
          futureQuestions.map(renderQuestionCard)
        ) : (
          <div>No future questions.</div>
        )}
      </div>
    </div>
  )
