...
const SubscriptionBadge = ({ sub, issubexp }) => {
    ...
    if (issubexp) return <span className={`${base} border-accent-amber text-accent-amber`}>Expired</span>
...
export default function AdminSubscriptions() {
    ...
    const [assignForm, setAssignForm] = useState({ userIdentifier: '', userId: '', planId: '', startDate: '', endDate: '', transactionId: '' })
    ...
    useEffect(() => {
        try {
            ...
            if (qUser) {
                setAssignForm({ userIdentifier: '', userId: qUser, planId: qPlan || '', startDate: '', endDate: '', transactionId: qTx || '' })
                setAssignOpen(true)
            }
        } catch (e) {
            // ignore
        ...
        <div className="p-6 space-y-6">
            ...
            <Table
                columns={[
                    ...
                    { header: 'Subscription', field: 'subscriptionPlan', render: (r) => <SubscriptionBadge sub={r.subscriptionPlan || r.subscription} issubexp={r.issubexp} /> },
                    { header: 'Subscribed On', field: 'subon', sortable: true, render: (r) => r.subon ? new Date(r.subon).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : '-' },
                    { header: 'Expires', field: 'subexp', sortable: true, render: (r) => r.subexp ? new Date(r.subexp).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : '-' },
                    { header: 'State', field: 'issubexp', sortable: true, render: (r) => r.issubexp ? 'Expired' : 'Active' },
                    { header: 'Actions', field: 'actions', render: (r) => (
                        <div className="inline-flex items-center justify-center gap-1">
                        <Button size="sm" onClick={() => handleRevoke(r._id)} variant="danger" disabled={actionLoading}>Revoke</Button>
                            {/* Only show Extend when user has an active Subscription doc and is not on the free plan */}
                            {r.hasSubscriptionDoc && !(r.subscriptionPlan && r.subscriptionPlan.slug === 'free') && (
                                <Button size="sm" variant="ghost" onClick={() => setExtendUser(r)} disabled={actionLoading}>Extend</Button>
                            )}
                            <Button size="sm" variant="ghost" onClick={() => { setAssignForm({ userIdentifier: r.email || r._id, userId: r._id, planId: '', startDate: '', endDate: '', transactionId: '' }); setAssignOpen(true) }} disabled={actionLoading}>Assign</Button>
                        </div>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-background/60 backdrop-blur-sm z-50">
                    <div className="bg-card-surface border border-soft rounded-2xl shadow-card p-6 w-80 space-y-4">
                        ...
                        <div className="flex justify-end gap-2">
                            <Button size="sm" onClick={() => setExtendUser(null)} variant="ghost">Cancel</Button>
            ...
            {/* Assign Modal */}
            <Modal open={assignOpen} onClose={() => { setAssignOpen(false); setAssignForm({ userIdentifier: '', planId: '', startDate: '', endDate: '', transactionId: '' }) }} title="Assign Subscription">
                <div className="space-y-3">
                    <div>
                        <label className="block text-text-secondary text-sm">User ID or Email</label>
                        <Input value={assignForm.userIdentifier} onChange={e => setAssignForm({...assignForm, userIdentifier: e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-text-secondary text-sm">Plan</label>
                        <select className="w-full p-2 border border-soft rounded-md" value={assignForm.planId} onChange={e => setAssignForm({...assignForm, planId: e.target.value})}>
                            <option value="">-- Select Plan --</option>
                            {plans.map(p=> <option key={p._id} value={p._id}>{p.name} — ₹{(p.finalPriceCents/100).toFixed(2)}</option>)}
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <Input value={assignForm.transactionId} onChange={e => setAssignForm({...assignForm, transactionId: e.target.value})} />
                    </div>
                    <div className="flex justify-end gap-2">
                            <Button size="sm" variant="ghost" onClick={() => { setAssignOpen(false); setAssignForm({ userIdentifier: '', userId: '', planId: '', startDate: '', endDate: '', transactionId: '' }) }}>Cancel</Button>
                        <Button
                            onClick={async ()=>{
                                // prefer explicit userId (set when opening from a row). If not present, only accept raw 24-char id input.
                                if(!assignForm.userId && !(/^[a-fA-F0-9]{24}$/.test((assignForm.userIdentifier||'').trim()))){
                                    toast.show('Please select a user from the table or enter a valid user id', { variant: 'error' })
                                    return
                                }
                                ...
                                try{
                                    ...
                                    setAssignOpen(false)
                                    setAssignForm({ userIdentifier: '', userId: '', planId: '', startDate: '', endDate: '', transactionId: '' })
                                    fetchUsers()
                                    toast.show('Subscription assigned', { variant: 'success' })
                                }catch(err){ toast.show(err?.response?.data?.message || err.message || 'Assign failed', { variant: 'error' }) }
                            }}
