...
export default function Analysis() {
  ...
  if (loading) return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-8"><Skeleton className="h-[70vh]" /><Skeleton className="h-[70vh]" /></div>
    </div>
  )
  if (error) return <div className="p-6 bg-error-red/10 border border-error-red/40 text-error-red rounded">{error}</div>
  if (!analysis) return null

  const { rc, questions, score, durationSeconds, attemptedAt, coverage, categoryStats, stats, isPersonalBest } = analysis
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-xl border border-soft bg-gradient-to-r from-primary/5 via-info-blue/5 to-success-green/5 p-6 md:p-8">
        <div className="flex flex-col gap-4">
          <div className="flex items-end gap-4">
            <div className="text-5xl font-bold text-success-green">{Math.round((score/questions.length)*100)}%</div>
            {isPersonalBest && <span className="px-2 py-1 rounded bg-primary-light text-white text-xs font-semibold">PERSONAL BEST</span>}
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            <StatTile label="Correct" value={`${correctCount}/${questions.length}`} icon={Target} iconColor="text-success-green" />
            <StatTile label="Time" value={`${Math.floor((durationSeconds||analysis.timeTaken)/60)}m ${(durationSeconds||analysis.timeTaken)%60}s`} icon={Clock3} iconColor="text-info-blue" />
            <StatTile label="Avg / Q" value={`${stats.avgTimePerQuestion}s`} icon={Zap} iconColor="text-primary" />
            <StatTile label="Speed Tier" value={stats.speedTier} icon={Activity} iconColor="text-primary" />
            {/* Date with ordinal format */}
            <StatTile
              label="Date"
              value={(() => {
                const d = new Date(attemptedAt)
                return `${getOrdinal(d.getDate())} ${d.toLocaleString('default',{month:'long'})} ${d.getFullYear()}`
              })()}
              icon={CalendarDays}
            ...
            {rc.topicTags?.length > 0 && (
              <div className="bg-white border border-soft rounded-lg p-4 shadow-sm flex flex-col">
                <div className="text-xs uppercase tracking-wide text-text-secondary mb-1">Topics</div>
                <div className="flex flex-wrap gap-2">
                  {rc.topicTags.map((t) => (
                    <span key={t} className="text-xs px-2 py-0.5 bg-info-blue/10 text-info-blue rounded-full">{t}</span>
                  ))}
                </div>
              </div>
            )}
      ...
      {/* Coverage Meter */}
      {coverage && <CoverageMeter coverage={coverage.coverage} target={0.7} taggedCount={coverage.taggedCount} totalWrong={coverage.incorrectCount} />}

      <div className="grid lg:grid-cols-5 gap-8 items-start">
        {/* Passage */}
        <div className="lg:col-span-2 space-y-6">
          <Card><CardHeader><h2 className="font-semibold text-text-primary">Passage</h2></CardHeader><CardContent><div className="prose max-w-none text-sm leading-relaxed whitespace-pre-line">{rc.passageText}</div></CardContent></Card>
            ...
            return (
              <Card key={i} className={q.isCorrect ? 'border-success-green/40' : 'border-error-red/40'}>
                ...
                <CardContent className="space-y-4">
                  ...
                  {!q.isCorrect&&<div className="pt-3 border-t border-soft"><div className="flex items-center gap-3"><ReasonTagSelect questionIndex={i} attemptId={analysis.attemptId} currentReason={current} onReasonSelected={code=>handleReasonSelected(i,code)}/>{!current&&<span className="text-[10px] text-text-secondary uppercase tracking-wide">Tag Reason</span>}</div></div>}
                </CardContent>
              </Card>
            )
          })()}
