...
export default function Login() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const msg =  err?.response?.status === 400 ? 'Invalid Credentials' : 'Login failed'
      setError(msg)
    ...
    <AuthShell title="Welcome back to ARYC">
      <div className="space-y-6">
        <form className="space-y-5" onSubmit={onSubmit}>
          ...
          <div className="flex items-center justify-between text-sm">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                className="w-4 h-4 rounded border-neutral-grey/30 text-text-primary focus:ring-primary"
              />
        ...
        <div className="text-center text-sm text-text-secondary">
          ...
          <Link to="/register" className="text-text-primary font-semibold hover:text-primary">
            Sign up
          </Link>
        </div>
      </div>
