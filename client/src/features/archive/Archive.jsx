...
export default function Archive() {
  ...
  if (error) return (
    <div className="min-h-screen flex flex-col p-6 bg-error-red/10 border border-error-red/40 text-error-red rounded">
      {error}
    </div>
  )
          ...
          <Card key={rc._id || rc.id}>
            <CardHeader className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {rc.attempted && (
                  <Button as="a" href={`/results/${rc._id || rc.id}`} variant="outline">View Results</Button>
                )}
