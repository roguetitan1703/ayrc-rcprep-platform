...
const getTodaysLeaderboard = async () => {
    ...
    return Attempt.aggregate([
        { $lookup: { from: 'rcpassages', localField: 'rcPassageId', foreignField: '_id', as: 'rcDetails' } },
        { $unwind: "$rcDetails" },
...

        {
            $addFields: {
                ...
                correctAnswers: { $size: { $filter: { input: { $ifNull: ["$q_details", []] }, as: "detail", cond: { $eq: ["$$detail.isCorrect", true] } } } }
            }
        },
        {
            $addFields: {
                attemptAccuracy: { $cond: { if: { $eq: ["$totalQuestions", 0] }, then: 0, else: { $divide: ["$correctAnswers", "$totalQuestions"] } } }
            }
        },
...
export async function leaderboard(req, res, next) {
    try {
        ...
        const totalEntries = results.today.length + Object.values(results.tagLeaderboards).flat().length + results.monthly.length;
        if (totalEntries === 0) {
